...
const PRESETS = [
  { id: 1, title: "Солнечно", model: "weather_models/Sun.glb" },
  { id: 2, title: "Облачно", model: "weather_models/Clouds.glb" },
  { id: 3, title: "Гроза", model: "weather_models/Lightning bolt.glb" },
  { id: 4, title: "Переменная облачность", models: [
      { model: "weather_models/Sun.glb" },
      { model: "weather_models/Clouds.glb" },
    ]
  }
];
...
function addCard(model, isUser, userId) {
  ...
  card.onclick = () => {
    if (isUser) {
      window.location.href = `weather_detail.html?user=${userId}`;
    } else {
      window.location.href = `weather_detail.html?id=${model.id}`;
    }
  };
