(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=class{constructor(e){this.parent=e}addListeners(e){document.getElementById(`back-button`).addEventListener(`click`,e)}getHTML(){return`
            ...
            <form id="metparam-form">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" step="0.1" class="form-control" id="value" name="value" 
                           value="${e.value!==void 0&&e.value!==null?e.value:``}" required>
                </div>
        ...
        `}bindNameChange(){let e=document.getElementById(`name`),t=document.getElementById(`unit`),n={Температура:`°C`,Давление:`мм.рт.ст`,Влажность:`%`,Ветер:`м/с`,"УФ излучение":`УФИ`};e&&t&&e.addEventListener(`change`,e=>{t.value=n[e.target.value]||``})}bindFormSubmit(){let e=document.getElementById(`metparam-form`);e&&e.addEventListener(`submit`,async e=>{e.preventDefault();let r={name:document.getElementById(`name`).value,value:parseFloat(document.getElementById(`value`).value),unit:document.getElementById(`unit`).value,description:document.getElementById(`description`).value,additionalData:this.metParam?.additionalData||{}};if(console.log(`Отправка данных:`,r),this.id){let e=await t.patch(n.updateMetParamById(this.id),r);console.log(`Результат PATCH:`,e),e.status===200?(alert(`Метеопараметр обновлён!`),this.clickBack()):alert(`Ошибка при обновлении: статус ${e.status}`)}else{let e=await t.post(n.createMetParam(),r);console.log(`Результат POST:`,e),e.status===201?(alert(`Метеопараметр создан!`),this.clickBack()):alert(`Ошибка при создании: статус ${e.status}`)}})}loadData(){if(this.id)(async()=>{console.log(`Загрузка данных для ID:`,this.id);let e=await t.get(n.getMetParamById(this.id));console.log(`Результат GET:`,e);let r=document.getElementById(`form-content`);e.status===200&&e.data?(this.metParam=e.data,r.innerHTML=this.getFormHTML(e.data),this.bindNameChange(),this.bindFormSubmit(),this.bindCancel()):r.innerHTML=`
                        <div class="alert alert-danger">
                            Ошибка загрузки данных. Метеопараметр не найден.<br>
                            Статус: ${e.status}
                        </div>
                    `})();else{let e=document.getElementById(`form-content`);e.innerHTML=this.getFormHTML(),this.bindNameChange(),this.bindFormSubmit(),this.bindCancel()}}bindCancel(){let e=document.getElementById(`cancel-btn`);e&&e.addEventListener(`click`,()=>{this.clickBack()})}clickBack(){new c(this.parent).render()}render(){this.parent.innerHTML=``,this.parent.insertAdjacentHTML(`beforeend`,this.getHTML()),new e(document.getElementById(`back-button-container`)).render(this.clickBack.bind(this)),this.loadData()}},i=class{constructor(e,t){this.parent=e,this.id=parseInt(t),this.data=null}getHTML(){return`
        ...
        `,document.getElementById(`edit-btn`).addEventListener(`click`,()=>{new r(this.parent,this.id).render()})}clickBack(){new c(this.parent).render()}render(){this.parent.innerHTML=``,this.parent.insertAdjacentHTML(`beforeend`,this.getHTML()),new e(document.getElementById(`back-button-container`)).render(this.clickBack.bind(this)),(async()=>{let{data:e,status:r}=await t.get(n.getMetParamById(this.id));r===200&&e?this.renderContent(e):document.getElementById(`day-content`).innerHTML=`
                    <div class="alert alert-danger">
                        Ошибка загрузки данных. Метеопараметр не найден.
                    </div>
                `})()}},a=class{constructor(e){this.parent=e}getHTML(e){return`
        ...
        `,this.parent.appendChild(e);let t=document.getElementById(`filter-container`);this.filterComponent=new s(t),this.filterComponent.render(this.applyFilter.bind(this));let n=document.getElementById(`grid-container`);if(this.grid=new o(n),this.grid.render(),window.searchFilterState){let{searchText:e}=window.searchFilterState;this.filterComponent.setValue(e),this.loadData(e)}else this.loadData();document.getElementById(`add-btn`).onclick=this.addCard.bind(this)}};new c(document.getElementById(`root`)).render();