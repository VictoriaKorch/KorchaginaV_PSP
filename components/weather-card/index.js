export class WeatherCardComponent {
    ...
    return (
        `
        <div class="weather-card" style="width: 200px; margin: 0; background: white; border-radius: 16px; box-shadow: 0 2px 8px rgba(0,0,0,0.05); border: 1px solid #f0f0f0;">
            <div class="card-body text-center" style="padding: 16px;">
                ...
                <!-- Кнопка с фоном как у страницы и черным текстом -->
    ...
    addListeners(data, listener) {
        // Ждем немного, чтобы кнопка точно появилась в DOM
        setTimeout(() => {
            const button = document.getElementById(`click-card-${data.id}`);
            if (button) {
                button.addEventListener("click", listener);
                console.log(`Обработчик добавлен для карточки ${data.id}`);
            } else {
                console.error(`Кнопка с id click-card-${data.id} не найдена`);
            }
        }, 0);
    }
